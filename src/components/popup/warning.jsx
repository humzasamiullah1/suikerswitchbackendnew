...
const WarningPopup = ({ name, itemId, onClose, onDelete }) => {
    ...
    <main className="w-full h-screen backdrop-blur-xs bg-black/80 fixed inset-0 z-50 flex items-center justify-center">
      <section className="w-[90%] sm:w-[65%] md:w-[50%] lg:w-[40%] xl:w-[30%] bg-texture myshades rounded-[31px] mx-auto">
        <div class="bg-white py-8 lg:py-5 rounded-xl justify-center items-center flex flex-col">
          <div className="flex justify-center w-[90%] pb-2 mx-auto border-b-2 border-gray-100">
            <h1 className="font-HelveticaNeueMedium text-center text-darkColor text-base">
              Are you want to delete this {name}?
            </h1>
          </div>
          <div className="flex justify-center mt-5">
            <button
              className="border text-xs rounded-full px-6 py-[10px] flex items-center font-HelveticaNeueMedium text-white bg-gkRedColor hover:bg-gkRedColor/90"
              onClick={onClose}
            >
              NO
            </button>
            <button className="border text-xs rounded-full px-6 py-[10px] flex items-center font-HelveticaNeueMedium text-white bg-green-600 hover:bg-green-600/90 ml-3" onClick={onSubmit}>
              <span>YES</span>
              {loading && (
