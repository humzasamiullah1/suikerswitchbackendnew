...
import { getHelpById, fetchUserById, deleteHelp } from "../utils/firebasefunctions.js";
...
const MainHelpDetail = () => {
  ...
  const fetchHelpElker = async () => {
    if (!id) return;

    const helpData = await getHelpById(id); // Function call
    setHelpDetail(helpData);
    fetchUserData(helpData.userId);
  };
  useEffect(() => {
    fetchHelpElker();
  }, [id]);
                  ...
                  <div className="absolute z-20 right-[-10px] top-[18px] mt-2 w-28 bg-white rounded-lg shadow-lg overflow-hidden transition-all duration-300 font-popinsMedium text-sm">
                    <ul className="py-2 pl-4 w-full">
                      <li
                        ...
                        onClick={() => openConfirmPopup(helpDetail.id)}
                      >
                        <Trash2 size={18} />
